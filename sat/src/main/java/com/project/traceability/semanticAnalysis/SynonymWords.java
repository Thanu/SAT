...
public class SynonymWords {
	...
	public static String[] getSynSetWords(String term) {
		...
		if (synsets.length > 0) {
			...
			//		 System.out.println(wordForms[j]  + j);
			//	}
	...
	public static void main(String args[]){
			checkSymilarity("accountNumber","account number");
	}

	public static boolean checkSymilarity(String term1, String term2) {
		// if the term cotains sub string "NO" at last then change the sub
		// string to "NUMBER"
		if (term1.substring(term1.length() - 2).equalsIgnoreCase("No")) {
			simpleWord1 = term1.replace(term1.substring(term1.length() - 2),
					"Number");
			// System.out.println("OOOOOOOOOOOOOOO"+simpleWord1);
		} else
			simpleWord1 = term1;
		if (term2.substring(term2.length() - 2).equalsIgnoreCase("No")) {
			simpleWord2 = term2.replace(term2.substring(term2.length() - 2),
					"Number");
			// System.out.println("PPPPPPPPPPPPPPP"+simpleWord2);
		} else
			simpleWord2 = term2;
		...
		System.out.println(simpleWord1 + " LLLLLLLLLL " + simpleWord2);

		// get similar words from WordNet dictionary
		String[] similarWordForTerm1 = getSynSetWords(simpleWord1);
		if(similarWordForTerm1 != null)
		for(int i = 0 ; i <similarWordForTerm1.length; i++)
			System.out.println(similarWordForTerm1[i]);
		String[] similarWordForTerm2 = getSynSetWords(simpleWord2);
		if(similarWordForTerm2 != null)
		for(int i = 0 ; i <similarWordForTerm2.length; i++)
			System.out.println(similarWordForTerm2[i]);

		// compare words which get from word net dictionary to get the
		// relationship files
		if (similarWordForTerm1 == null && similarWordForTerm2 != null) {
			...
			for (int i = 0; i < similarWordForTerm1.length; i++) {
				for (int j = 0; j < similarWordForTerm2.length; j++) {
					if (similarWordForTerm1[i]
							.equalsIgnoreCase(similarWordForTerm2[j])) {
						System.out.println(similarWordForTerm1[i] + "OOOOOOO"
								+ similarWordForTerm2[j]);
						status = true;
						break;
					}
				}
		...
		// LevenshteinDistance.similarity(term1, term2)>.85){
		//
		// System.out.println(term1.equalsIgnoreCase(term2)+" : "+LevenshteinDistance.similarity(term1,
		// term2)+" : "+ term1+"**************"+term2+"TRUE");
		// return true;
		// }
		// else
		// return false;
	...
	public static boolean isFirstletterChanged(String term1, String term2) {
		if (term1.substring(1).equalsIgnoreCase(term2.substring(1))
				&& term1.charAt(0) != term2.charAt(0)) {
