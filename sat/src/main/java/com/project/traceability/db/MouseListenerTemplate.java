...
public class MouseListenerTemplate implements PreviewMouseListener {
	...
	public void mouseClicked(PreviewMouseEvent event,
		...
		for (Node node : Lookup.getDefault().lookup(GraphController.class)
				.getModel(workspace).getGraph().getNodes()) {
			if (clickingInNode(node, event)) {
				properties.putValue("display-label.node.id",
						node.getNodeData().getId());
				System.err.println("Node "
						+ node.getNodeData().getLabel()
						+ " clicked!");// System.out is ignored in Netbeans
